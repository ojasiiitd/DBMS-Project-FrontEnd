...
def queries() :
    ...
    data.append(cur.fetchall())
    cur.execute("Select Count(Students.RollNo), Branch.BranchID, Branch.Name From Students INNER JOIN Branch ON Students.BranchID=Branch.BranchID Group By Branch.BranchID")
    data.append(cur.fetchall())
    cur.execute("Select Students.RollNo, TAsin.CourseCode from Students LEFT JOIN  TAsin ON Students.RollNo=TAsin.RollNo")
    data.append(cur.fetchall())
    cur.execute("Select Requests_Complaints.RequestID,Requests_Complaints.CurrStatus, Students.First_Name from Students Right join Requests_Complaints  on Students.RollNo=Requests_Complaints.RollNo where CurrStatus=\"InProgress\"")
    data.append(cur.fetchall())
    cur.execute("Select a.CourseCode, a.PrereqCode,b.PrereqCode from prereq a, prereq b where a.PrereqCode=b.CourseCode")
    data.append(cur.fetchall())
    cur.execute("SELECT S1.RollNo, CONCAT(S1.First_Name, ' ', S1.Last_Name) Name, S1.Hostel_ID FROM Students S1 INNER JOIN Students S2 on S2.RollNo = '2018776' AND S2.Hostel_ID = S1.Hostel_ID")
    data.append(cur.fetchall())
    cur.execute("SELECT R.RollNo, First_Name, Last_Name, Hostel_ID FROM Requests_Complaints R LEFT JOIN Students S ON R.RollNo = S.RollNo")
    data.append(cur.fetchall())
    cur.execute("Select count(TAsin.RollNo), Courses.FacultyID from TAsin INNER JOIN Courses ON Courses.CourseCode=TAsin.CourseCode group by Courses.FacultyID")
    data.append(cur.fetchall())
    cur.execute("Select Students.MessID,Students.RollNo,Mess.Menu from Students CROSS JOIN Mess ON Students.MessID=Mess.MessID LIMIT 10")
    data.append(cur.fetchall())
    cur.execute("Select takes.CourseCode ,takes.Grade, Students.RollNo, Students.First_Name,Students.Last_Name from takes JOIN Students ON takes.RollNo=Students.RollNo")
    data.append(cur.fetchall())
    cur.execute("DROP VIEW IF EXISTS `studentcourses`")
    cur.execute("create view studentcourses as select Students.First_Name,Students.Last_Name,Students.RollNo,takes.CourseCode,Students.CurrentSemester from Students join takes on Students.RollNo=takes.RollNo")
    cur.execute("select T.First_Name,T.Last_Name,T.RollNo,T.CourseCode from studentcourses as T join studentcourses s  where s.First_Name=\"Kariane\" and T.CurrentSemester=s.CurrentSemester and T.First_Name!=\"Kariane\" and T.CourseCode=\"CSE345\"")
    data.append(cur.fetchall())
    cur.execute("select * from   (select Students.First_Name,Students.Last_Name,Students.RollNo,takes.CourseCode from Students join takes on Students.RollNo=takes.RollNo)   as T join  (select students.First_Name,Last_Name,students.RollNo,takes.CourseCode from Students students join takes on students.RollNo=takes.RollNo)    as R where R.RollNo  = T.RollNo  and R.CourseCode != T.CourseCode")
    data.append(cur.fetchall())
